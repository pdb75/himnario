...
class _DescargadosPageState extends State<DescargadosPage> {
  ...
  void initDB([bool refresh = true]) async {
    ...
    for (dynamic favorito in favoritosQuery) favoritos.add(favorito['himno_id']);
    for (dynamic himno in data) {
      himnos.add(Himno(
          numero: himno['id'], titulo: himno['titulo'], transpose: himno['transpose'], descargado: true, favorito: favoritos.contains(himno['id'])));
    }
    await db.close();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: Scroller(
            himnos: himnos, cargando: cargando, initDB: initDB, mensaje: 'No has descargado ningún himno\n para escuchar la melodia sin conexión'));
  }
}
